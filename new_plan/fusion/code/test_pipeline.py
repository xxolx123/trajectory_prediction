...
整体流程：
  1) 加载 fusion/config.yaml 构出 FullNetV2（按 enable 状态）。
     默认全部子网络（lstm1 / gnn1 / constraint_optimizer / lstm2 / gnn2）启用，
     `fusion/config.yaml` 中各 ckpt 字段默认指向 checkpoints 目录，
  ...
  2) 从 gnn1 的 npz 数据里抽 N 条样本：
       ...
       - eta：GNN2 启用时按 gnn2/config.yaml 的 [eta_min_sec, eta_max_sec] 随机
              采样（per-sample，rng 由 --seed 派生）；GNN2 关闭时填全 0 哨兵
...
输出（写到 fusion/eval_vis/run_<split>_<timestamp>/<mode>/，两种 mode 共享父目录）::
  inputs.npz             FullNetV2 实际入参（hist_traj + 7 个 ctx 张量，含真 eta）
  ...
  vis_6_intent_threat.png            top-K 候选的 LSTM2 意图 + 威胁度柱状图
  vis_7_strike_zone.png              top-K 候选的 GNN2 打击区域：圆心=strike_pos.xy，
                                      半径=strike_radius，填充透明度∝strike_conf
                                      （仅 GNN2 启用时生成）
  meta.txt               文字摘要（enable / ckpt / 哨兵值校验 / mode_prob /
                                    gnn2 启用时附 eta + radius/conf 统计）
...
# ---- 图 6 用：intent 类别色板（4 类 + 哨兵）----
_INTENT_COLORS = ["#1F77B4", "#FF7F0E", "#2CA02C", "#D62728"]   # C0..C3
_INTENT_SENTINEL_COLOR = "#999999"


def vis_intent_threat(
    intent: np.ndarray,            # [B, K]   float32  (lstm2 关时全 = -1)
    threat: np.ndarray,            # [B, K]   float32  (lstm2 关时全 = NaN)
    mode_prob: np.ndarray,         # [B, K]
    chosen: List[int],
    task_type: np.ndarray, type_id: np.ndarray,
    n_intent_classes: int,
    lstm2_enabled: bool,
    out_path: Path, title_suffix: str,
) -> None:
    """
    图 6：每个样本一个子图，横向 K 条柱：
      柱高     = threat_prob ∈ [0, 1]
      柱颜色   = intent_class（按 _INTENT_COLORS 映射；哨兵 -1 用灰色）
      柱顶文字 = "C{class}  p={mode_prob:.0%}"

    便于直接看出"top-K 候选各自的意图类别 + 威胁度概率 + GNN1 概率权重"。
    """
    B, K = intent.shape
    fig, axes, rows, cols = _make_grid_fig(B, base_w=5.0, base_h=4.2)
    for i in range(rows * cols):
        r, c = i // cols, i % cols
        ax = axes[r, c]
        if i >= B:
        ...
        for k in range(K):
            t = float(threats_i[k]) if np.isfinite(threats_i[k]) else 0.0
            cls = int(intents_i[k]) if np.isfinite(intents_i[k]) and intents_i[k] >= 0 else -1
            if cls >= 0 and cls < n_intent_classes:
                color = _INTENT_COLORS[cls % len(_INTENT_COLORS)]
                cls_label = f"C{cls}"
            else:
                color = _INTENT_SENTINEL_COLOR
                cls_label = "OFF"
            ax.bar(
                xs[k], t, width=0.7,
            ...
            # 柱顶 / 柱内文字
            txt = f"{cls_label}\nthreat={t:.2f}\np={float(probs_i[k]) * 100:.1f}%"
            ax.text(
                xs[k], min(t + 0.04, 1.02), txt,
                ha="center", va="bottom", fontsize=8, zorder=4,
            )

        ax.set_xticks(xs)
        ax.set_xticklabels([f"top{k + 1}" for k in range(K)], fontsize=9)
        ax.set_ylim(0.0, 1.30)   # 多留点空给柱顶文字
        ax.set_ylabel("threat_prob")
        ax.axhline(0.5, linestyle=":", color="#666666", linewidth=0.7, alpha=0.6, zorder=1)
    ...
    # 整图共用 legend：4 类 intent + 哨兵
    legend_handles = []
    for cls in range(min(n_intent_classes, len(_INTENT_COLORS))):
        legend_handles.append(
            Line2D([0], [0], marker="s", linestyle="",
                   markerfacecolor=_INTENT_COLORS[cls], markeredgecolor="black",
                   markersize=10, label=f"C{cls}")
        )
    if not lstm2_enabled:
        legend_handles.append(
            Line2D([0], [0], marker="s", linestyle="",
                   markerfacecolor=_INTENT_SENTINEL_COLOR, markeredgecolor="black",
                   markersize=10, label="OFF（lstm2 禁用）")
        )
    fig.legend(handles=legend_handles, loc="lower center",
               ncol=max(4, len(legend_handles)), fontsize=9, frameon=False)

    fig.suptitle(
        f"图 6 / intent_class + threat_prob（per top-K）  |  {title_suffix}",
        fontsize=12,
    )
...
# ============================================================
# 图 7：GNN2 打击区域（单场景，1×K 子图分别展示每条 top-K 的圆 + 信息）
# ============================================================

def vis_strike_zone(
    hist_xy: np.ndarray,                # [Tin, 2]
    gt_xy: Optional[np.ndarray],        # [Tout, 2] 或 None
    refined_topk_xy: np.ndarray,        # [K, T, 2]
    mode_prob_row: np.ndarray,          # [K]
    strike_pos: np.ndarray,             # [K, 3]
    strike_radius: np.ndarray,          # [K]
    strike_conf: np.ndarray,            # [K]
    eta_value: int,
    position: np.ndarray,               # [3]
    sample_id: int,
    task_type: int, type_id: int,
    out_path: Path, title_suffix: str,
) -> None:
    """
    图 7：单一场景下分 K 个子图，每个子图高亮一条 top-K 的"打击区域"。

    每个子图布局（K 个共享视口）::
      - 当前 top-K 候选的轨迹用粗彩线 + 实心 strike 圆（半径=strike_radius，
        填充 alpha 由 strike_conf 决定）+ ×号圆心 + 子图标题文字
      - 其它 K-1 条候选用浅灰色 0.6 线宽细线 + 同色无填充薄圆（仅作上下文）
      - history（黑实线）/ GT future（黑虚线）/ position（红五角星）/ 原点星

    避免和 4-子图版本一样把 K 个圆挤在一起、文本互相覆盖。
    """
    K = int(refined_topk_xy.shape[0])

    # 共享视口：包含 history + 全部 K 条轨迹 + 全部 strike 圆 + position + 原点
    arrs: List[np.ndarray] = [
        hist_xy, refined_topk_xy.reshape(-1, 2),
        np.zeros((1, 2)), position[None, :2],
    ]
    if gt_xy is not None:
        arrs.append(gt_xy)
    for k in range(K):
        if not np.isfinite(strike_pos[k]).all() or not np.isfinite(strike_radius[k]):
            continue
        cx, cy = float(strike_pos[k, 0]), float(strike_pos[k, 1])
        rad = max(float(strike_radius[k]), 0.0)
    ...
    fig, axes = plt.subplots(1, K, figsize=(5.5 * K, 5.0 + 0.6))
    if K == 1:
        axes = np.array([axes])

    for k_focus in range(K):
        ax = axes[k_focus]

        # ---- 背景：其它 top-K（浅灰线 + 无填充薄圆） ----
        for k_bg in range(K):
            if k_bg == k_focus:
                continue
            xy = refined_topk_xy[k_bg]
            ax.plot(xy[:, 0], xy[:, 1], color="#BBBBBB", linewidth=1.0,
                    alpha=0.85, zorder=2)
            if (np.isfinite(strike_pos[k_bg]).all()
                    and np.isfinite(strike_radius[k_bg])):
                cx_b, cy_b = float(strike_pos[k_bg, 0]), float(strike_pos[k_bg, 1])
                rad_b = max(float(strike_radius[k_bg]), 0.0)
                ax.add_patch(Circle(
                    (cx_b, cy_b), rad_b,
                    facecolor="none", edgecolor="#999999", linewidth=0.9,
                    linestyle="--", alpha=0.7, zorder=3,
                ))
                ax.scatter(cx_b, cy_b, marker="x", color="#999999", s=24,
                           linewidths=1.0, zorder=3)

        # ---- 公共背景：hist / GT / position / 原点 ----
        _draw_history(ax, hist_xy, with_label=(k_focus == 0))
        _draw_gt_future(ax, gt_xy, with_label=(k_focus == 0))
        _draw_position(ax, position, (xmin, xmax), (ymin, ymax))

        # ---- 主角：当前 top-K 候选 ----
        col = _TOPK_COLORS[k_focus % len(_TOPK_COLORS)]
        focus_xy = refined_topk_xy[k_focus]
        ax.plot(focus_xy[:, 0], focus_xy[:, 1], color=col, linewidth=2.4,
                alpha=1.0, zorder=5,
                label=f"top{k_focus + 1}  p={float(mode_prob_row[k_focus]) * 100:.1f}%",
                solid_capstyle="round")

        if (np.isfinite(strike_pos[k_focus]).all()
                and np.isfinite(strike_radius[k_focus])
                and np.isfinite(strike_conf[k_focus])):
            cx = float(strike_pos[k_focus, 0])
            cy = float(strike_pos[k_focus, 1])
            rad = max(float(strike_radius[k_focus]), 0.0)
            conf_v = float(strike_conf[k_focus])
            face_alpha = float(np.clip(0.10 + 0.55 * conf_v, 0.10, 0.65))
            ax.add_patch(Circle(
                (cx, cy), rad,
                facecolor=col, edgecolor=col, linewidth=1.8,
                alpha=face_alpha, zorder=6,
            ))
            ax.scatter(cx, cy, marker="x", color=col, s=58,
                       linewidths=2.4, zorder=7,
                       label=f"strike center ({cx:.2f}, {cy:.2f})")
            stat_str = (
                f"r = {rad:.3f} km\n"
                f"conf = {conf_v * 100:.1f}%\n"
                f"p (gnn1) = {float(mode_prob_row[k_focus]) * 100:.1f}%"
            )
        else:
            stat_str = "strike_* = NaN"

        # 子图右下角统一显示该 top-K 的关键数值（不和轨迹打架）
        ax.text(
            0.985, 0.025, stat_str,
            fontsize=10, color=col, zorder=8,
            ha="right", va="bottom", transform=ax.transAxes,
            bbox=dict(facecolor="white", edgecolor=col, alpha=0.85,
                      linewidth=1.0, pad=4.0),
        )

        _finalize_ax(
            ax,
            f"top{k_focus + 1}   eta = {eta_value} s",
            (xmin, xmax), (ymin, ymax),
        )

    fig.suptitle(
        f"图 7 / GNN2 打击区域（单场景 sample {sample_id}  task={task_type}  "
        f"type={type_id}  eta={eta_value}s）  |  {title_suffix}",
        fontsize=12,
    )
...
def save_inputs_txt(
    ...
    blocks = [
        ...
        _array_to_txt_block(
            eta_np,
            "eta  [B]  long  (秒；GNN2 启用时按 [eta_min,eta_max] 随机采样，关闭时全 0)",
        ),
        "",
    ]
...
def run(
    ...
    """
    mode:
      - "with_road": 完整 2-pass 流程：先无路网拿 unrefined top-K → 用它合成路网 →
                     再喂回去拿 refined。0..59 列 = refined。
      - "no_road":   单 pass，永远不喂路网。0..59 列 = GNN1 物理轨迹（unrefined）。
                     产物里仍写 inputs.npz / outputs.npz / vis_*.png，但路网相关
                     图层为空，等同部署侧"无路网"分支的输出形态。

    输出落到 parent_run_dir / mode / ...，由调用方（main）保证两种模式共享同一个
    parent_run_dir，便于把同一次实验的两份产物对照阅读。
    """
    if mode not in ("no_road", "with_road"):
        raise ValueError(f"mode 必须是 'no_road' / 'with_road'，得到 '{mode}'")
    # ---- device ----
    ...
    # ---- ETA：GNN2 启用时按 gnn2/config.yaml 的 [eta_min_sec, eta_max_sec] 随机采样 ----
    # gnn2 关闭时填全 0；fusion 内部仍当合法输入处理（哨兵列另填 NaN）
    gnn2_enabled_for_run = bool(model.enable_flags.get("gnn2", False))
    if gnn2_enabled_for_run:
        gnn2_cfg_path = _resolve_rel(
            patched_cfg.get("gnn2", {}).get("config", ""), fusion_cfg_dir,
        )
        with gnn2_cfg_path.open("r", encoding="utf-8") as f:
            gnn2_cfg = yaml.safe_load(f)
        gnn2_data = gnn2_cfg.get("data", {}) or {}
        eta_min_sec = int(gnn2_data.get("eta_min_sec", 0))
        eta_max_sec = int(gnn2_data.get("eta_max_sec", 600))
        eta_rng = np.random.default_rng(int(seed) * 31 + 17)
        eta_np = eta_rng.integers(
            eta_min_sec, eta_max_sec + 1, size=B, dtype=np.int64,
        )
        print(
            f"[Test/Pipeline] gnn2 ON  eta sampled in [{eta_min_sec},{eta_max_sec}] s "
            f"per sample = {eta_np.tolist()}"
        )
    else:
        eta_min_sec, eta_max_sec = 0, 0
        eta_np = np.zeros(B, dtype=np.int64)
        print("[Test/Pipeline] gnn2 OFF eta = 0（哨兵；fusion 内部跳过 GNN2）")
    ...
    if mode == "with_road":
        ...
        print(
            f"[Test/Pipeline] road_points = {tuple(rp.shape)}, mask True = {n_valid_pts}"
        )
        ...
        ctx_with_road = ContextBatch(
            ...
            type=torch.from_numpy(type_id).to(device),
            position=torch.from_numpy(position).to(device),
            road_points=rp, road_mask=rm, eta=eta_t,
        )
        with torch.no_grad():
        ...
        # no_road：完全跳过路网，0..59 列直接是 GNN1 物理轨迹（= unrefined）。
        # 产物里 road_points / road_mask 用零占位，与部署侧"无路网"输入形态一致。
        print("[Test/Pipeline] mode=no_road → 跳过 Pass 2，refined = GNN1 物理轨迹")
        rp = torch.zeros(
            B, nb_max, np_max, 3, dtype=torch.float32, device=device,
        )
        rm = torch.zeros(B, nb_max, np_max, dtype=torch.bool, device=device)
        branches_enu_per_sample = [[] for _ in range(B)]
        out_full = out_no_road.cpu().numpy()                          # [B, K, 68]
        traj_refined = traj_no_road.copy()                            # 已 reshape 过
    ...
    # eta_np 在前文已按 gnn2.enable 状态准备好（启用时随机采样、关闭时全 0）
    np.savez(
        run_dir / "inputs.npz",
    ...
    meta_lines = [
        ...
        f"  lstm2.ckpt   = {patched_cfg.get('lstm2', {}).get('ckpt', '')}",
        f"  gnn2.ckpt    = {patched_cfg.get('gnn2', {}).get('ckpt', '')}",
        "",
        "哨兵值校验：",
    ...
    # GNN2 段：启用时给出 eta 采样范围 / per-sample eta / strike_radius / strike_conf 统计
    if gnn2_enabled_for_run:
        radius_mean_per = np.nanmean(strike_radius, axis=1)             # [B]
        conf_mean_per = np.nanmean(strike_conf, axis=1)                 # [B]
        meta_lines += [
            "",
            "gnn2 stats（GNN2 启用）：",
            f"  eta range used   = [{eta_min_sec}, {eta_max_sec}] s",
            f"  per-sample eta   = {eta_np.tolist()}",
            f"  strike_radius mean (top-K avg, per sample) = "
            + ", ".join(f"{v:.3f}" for v in radius_mean_per.tolist()) + " km",
            f"  strike_conf   mean (top-K avg, per sample) = "
            + ", ".join(f"{v:.3f}" for v in conf_mean_per.tolist()),
            f"  strike_radius global mean = "
            f"{float(np.nanmean(strike_radius)):.3f} km",
            f"  strike_conf   global mean = "
            f"{float(np.nanmean(strike_conf)):.3f}",
        ]
    else:
        meta_lines += [
            "",
            "gnn2 stats：GNN2 OFF，strike_pos / strike_radius / strike_conf 列填 NaN 哨兵",
        ]
    ...
    # 图 6：每个样本 K 条 top-K 候选的意图分类 + 威胁度（柱状图）
    vis_intent_threat(
        intent=intent, threat=threat, mode_prob=mode_prob_v2,
        chosen=chosen, task_type=task_type, type_id=type_id,
        n_intent_classes=int(model.n_intent_classes),
        lstm2_enabled=bool(model.enable_flags.get("lstm2", False)),
        out_path=run_dir / "vis_6_intent_threat.png",
        title_suffix=title_suffix,
    )
    print(f"[Test/Pipeline] saved vis_6_intent_threat.png")

    # 图 7：GNN2 打击区域（单场景 1×K 子图；仅 GNN2 启用时生成）
    # 选样本启发式：取 hist + top-K 轨迹的 xy 包围盒"最方正"（min(x_range,y_range)
    # 最大）的样本。轨迹接近纯垂直 / 纯水平时 viewport 非常细长，strike 圆挤在
    # 一起难看；尽量挑一个散得开的场景。
    if bool(model.enable_flags.get("gnn2", False)):
        def _aspect_score(b: int) -> float:
            pts = np.concatenate(
                [hists_xy[b], refined_topk_xy[b].reshape(-1, 2)],
                axis=0,
            )
            x_range = float(pts[:, 0].max() - pts[:, 0].min())
            y_range = float(pts[:, 1].max() - pts[:, 1].min())
            return min(x_range, y_range)

        sample_idx_for_strike = int(
            np.argmax([_aspect_score(b) for b in range(B)])
        )
        print(
            f"[Test/Pipeline] vis_7 picks sample idx={sample_idx_for_strike} "
            f"(raw_idx={chosen[sample_idx_for_strike]})"
        )
        vis_strike_zone(
            hist_xy=hists_xy[sample_idx_for_strike],
            gt_xy=gt_xys[sample_idx_for_strike],
            refined_topk_xy=refined_topk_xy[sample_idx_for_strike],
            mode_prob_row=mode_prob_v2[sample_idx_for_strike],
            strike_pos=strike_pos[sample_idx_for_strike],
            strike_radius=strike_radius[sample_idx_for_strike],
            strike_conf=strike_conf[sample_idx_for_strike],
            eta_value=int(eta_np[sample_idx_for_strike]),
            position=position[sample_idx_for_strike],
            sample_id=int(chosen[sample_idx_for_strike]),
            task_type=int(task_type[sample_idx_for_strike]),
            type_id=int(type_id[sample_idx_for_strike]),
            out_path=run_dir / "vis_7_strike_zone.png",
            title_suffix=title_suffix,
        )
        print(f"[Test/Pipeline] saved vis_7_strike_zone.png")
    else:
        print("[Test/Pipeline] gnn2 OFF, skip vis_7_strike_zone.png")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="部署 ONNX 双形态对应：no_road = 5 输入版（不走路网）；"
             "with_road = 7 输入版（含路网约束，2-pass 真路网合成）；"
             "both（默认）= 两种各跑一次，分别落到 run_<split>_no_road_<stamp>/"
             "和 run_<split>_with_road_<stamp>/",
    )
    ...
    modes_to_run: List[str] = (
        ["no_road", "with_road"] if args.mode == "both" else [args.mode]
    )

    # 两种 mode 共享同一个时间戳父目录，下面分 no_road / with_road 子目录
    # 方便把同一次实验的两份产物对照阅读：
    #   run_<split>_<stamp>/
    #     ├── no_road/   inputs.npz / outputs.npz / meta.txt / vis_*.png
    #     └── with_road/ inputs.npz / outputs.npz / meta.txt / vis_*.png
    stamp = time.strftime("%Y%m%d_%H%M%S")
    parent_run_dir = out_root / f"run_{args.split}_{stamp}"
    parent_run_dir.mkdir(parents=True, exist_ok=True)
    print(f"[Test/Pipeline] parent run_dir = {parent_run_dir}")

    for run_mode in modes_to_run:
        print(f"\n{'=' * 60}\n[Test/Pipeline] === mode = {run_mode} ===\n{'=' * 60}")
        run(
