...
流程（inference only）：
    ...
    hist -> OutlierFilter -> LSTM1 -> M 条候选 [B,M,T,6]（归一化+delta）
        ...
        (task_type / type / position) → GNN1
                          ...
                          topk(K) + renorm sum=1
                                        |
              top-K 归一化候选 → 反归一化 + cumsum → 物理绝对坐标 [B, K, T, 6]
                                        |
                        展平成 [B*K, T, 6] 走下游三次
                                        |
...
class FullNetV2(nn.Module):
    ...
    def forward(self, x_raw: torch.Tensor, ctx: ContextBatch) -> torch.Tensor:
        ...
        probs_all = gnn1_out["probs"]   # [B, M]

        # 4) top-K 选择 + 重归一化到 sum=1
        K = self.top_k
        top_p_raw, top_idx = torch.topk(probs_all, k=K, dim=-1)   # [B, K]
        top_probs = top_p_raw / top_p_raw.sum(dim=-1, keepdim=True).clamp_min(1e-8)

        # 5) gather top-K 归一化候选 → 反归一化到物理绝对坐标
        idx_exp = (
            top_idx[..., None, None]
            .expand(B, K, Tout, Df)
        )
        ...
        # 6) 展平到 B*K，走下游（ConstraintOptimizer → LSTM2 → GNN2）
