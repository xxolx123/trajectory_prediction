...
class FullNetV2(nn.Module):
    ...
    def forward(self, x_raw: torch.Tensor, ctx: ContextBatch) -> torch.Tensor:
        ...
        # GNN2 只吃 (路网约束后轨迹, eta)；意图特征不再喂给打击区域分支
        strike_out = self.gnn2(refined_flat, ctx_flat.eta)
        strike_pos = strike_out["strike_pos"]        # [B*K, 3]   km xyz
        strike_radius = strike_out["strike_radius"]  # [B*K, 1]   km
        strike_conf = strike_out["strike_conf"]      # [B*K, 1]   [0,1]

        # 6) 拼 [B, K, 68]
        # 输出轨迹用"路网约束后" refined（满足业务需求：3 条预测轨迹必须贴路）
