...
- LSTM2 的 11 维 StandardScaler（mean_lstm2 / std_lstm2）现在以 register_buffer
  形式打包进 ONNX，导出后是图内常量，C++ 侧无需感知/读取额外的 scaler 文件。
  同理 LSTM1 的 mean_A / std_A 也是 buffer，与之前行为一致。
- 默认 ONNX opset = 17（fusion/code/export_onnx.py 默认值）：
    * opset ≥ 14：LSTM2 Transformer 的 scaled_dot_product_attention 算子
    * opset ≥ 16：ConstraintOptimizer 的 searchsorted 算子
    * opset 17：取较新带保障版本，ORT 1.13+（2022 年起）广泛支持
  如果部署环境只能用 opset ≤ 13：lstm2/code/train/model.py 末尾保留了一份
  "手写 attention" 的 IntentTransformerManual 实现（默认注释掉），与现有
  IntentTransformer 数学等价、state_dict 完全兼容；解开后可导出到更低 opset。
- 已知遗留问题：当前 ONNX 实际只有 4 个 input（hist_traj / task_type / type /
  position），road_points / road_mask / eta 被 trace 优化掉了。原因是
  ConstraintOptimizer 的 _road_arc_projection 使用了 Python for 循环、.item()、
  布尔索引、torch.searchsorted 等 trace 不友好写法，dummy 输入走全 0 时整个
  路网分支被消除。要拿到完整 7-input ONNX 需独立任务对 ConstraintOptimizer
  做向量化重构。lstm2 接入本身完全无关，已全部正常打包进 ONNX（含 buffer 与
  Transformer 权重）。
