...
可视化 generate_trajs.py 生成的轨迹数据（synthetic_trajectories.csv）。
...
用法示例（在项目根目录）：
  # 默认：每种类型随机画 1 条
  python code/data/visualize_trajs.py --csv data/raw/synthetic_trajectories.csv

  # 叠加模式：每种类型把所有轨迹都画在一个子图
  python code/data/visualize_trajs.py --csv data/raw/synthetic_trajectories.csv --overlay-all

  # 改随机种子
  python visualize_trajs.py --csv data/raw/synthetic_trajectories.csv --seed 123
"""
...
def plot_one_traj_on_axis(ax,
    ...
    参数：
      ax        : matplotlib Axes
      traj_info : load_trajectories 返回的某个 traj_dict[traj_id]
      title     : 子图标题
      line_kw   : 轨迹线条的样式 dict（可选）
      start_kw  : 起点散点的样式 dict（可选）
      end_kw    : 终点散点的样式 dict（可选）
    """
...
def visualize_per_type(csv_path: str,
                       seed: int = 42,
    ...
    if seed is not None and seed >= 0:
        # 显式给了 seed，就固定下来，方便复现
        random.seed(seed)
    else:
        # 不给 seed 或 seed<0，就让系统自己选种子（每次都不一样）
        random.seed()
    ...
    for idx, t in enumerate(traj_types):
        ...
        if overlay_all:
            ...
            plot_one_traj_on_axis(
                ...
                line_kw={"linewidth": 2.0, "alpha": 1.0,
                         "color": "tab:blue"},
            )
...
def main():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default="data/raw/synthetic_trajectories.csv",
        help="轨迹 CSV 文件路径（默认：data/raw/synthetic_trajectories.csv）",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=-1,
        help="随机种子；>=0 时固定结果；<0 时每次运行都随机（默认）",
    )
    parser.add_argument(
        "--overlay-all",
        action="store_true",
