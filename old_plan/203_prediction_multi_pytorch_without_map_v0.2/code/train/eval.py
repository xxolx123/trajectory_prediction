...
def evaluate_loader(
    ...
    with torch.no_grad():
        for inputs_norm, targets_norm in loader:
            ...
            t0 = time.time()
            pred_trajs_norm, mode_logits = model(inputs_norm)
            if device.type == "cuda":
                torch.cuda.synchronize()
            ...
            # 调用 MTP 损失（包含回归 + 分类），主要是看数值是否正常
            _ = loss_fn(pred_trajs_norm, mode_logits, targets_norm)
...
def plot_random_trajectory(
    ...
    # 计算该样本上各 mode 概率
    probs = torch.softmax(mode_logits[0], dim=0).cpu().numpy()  # [M]
